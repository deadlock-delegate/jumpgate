...
from portal_crypto.constants import (
    TRANSACTION_DELEGATE_REGISTRATION,
    TRANSACTION_MULTI_SIGNATURE_REGISTRATION,
    TRANSACTION_SECOND_SIGNATURE_REGISTRATION,
    TRANSACTION_VOTE,
)
from portal_crypto.exceptions import PortalInvalidTransaction
from portal_crypto.schnorr import schnorr
from portal_crypto.transactions.deserializer import Deserializer
from portal_crypto.transactions.serializer import Serializer
...
class Transaction(object):
    ...
    def get_id(self):
        ...
        return sha256(
            self.to_bytes(
                ...
                skip_second_signature=False,
                skip_multi_signature=False,
            )
        ).hexdigest()
    ...
    def deserialize(self, serialized):
        ...
        Returns:
            portal_crypto.transactions.transaction.Transaction: Transaction
        """
    ...
    def verify_schnorr(self):
        ...
        returns True
        """
        is_valid = schnorr.b410_schnorr_verify(
            self.to_bytes(), self.senderPublicKey, self.signature
        )

        if not is_valid:
            raise PortalInvalidTransaction("Transaction could not be verified")
    ...
    def verify_schnorr_secondsig(self, secondPublicKey):
        ...
        """
        is_valid = schnorr.b410_schnorr_verify(
            self.to_bytes(False, True), secondPublicKey, self.signSignature
        )
    ...
    def verify_schnorr_multisig(self):
        ...
        """
        is_valid = schnorr.b410_schnorr_verify(
            self.to_bytes(True, True, False), self.senderPublicKey, self.signature
        )
