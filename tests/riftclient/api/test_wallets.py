...
def test_all_calls_correct_url_with_default_params():
    responses.add(
        responses.GET, "http://127.0.0.1:4002/wallets", json={"success": True}, status=200
    )
...
def test_top_calls_correct_url_with_default_params():
    responses.add(
        responses.GET, "http://127.0.0.1:4002/wallets/top", json={"success": True}, status=200
    )
...
def test_locks_calls_correct_url_with_default_params():
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == ("http://127.0.0.1:4002/wallets/12345/locks?limit=100")
...
def test_votes_calls_correct_url_with_default_params():
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url == ("http://127.0.0.1:4002/wallets/12345/votes?limit=100")
...
def test_votes_calls_correct_url_with_passed_in_params():
    ...
    assert len(responses.calls) == 1
    assert responses.calls[0].request.url.startswith("http://127.0.0.1:4002/wallets/12345/votes?")
    assert "page=5" in responses.calls[0].request.url
    assert "limit=69" in responses.calls[0].request.url
